...
import com.vension.mvp.beans.headline.NewsChannelBean;
import com.vension.mvp.db.sqlite.DatabaseHelper;
import com.vension.mvp.db.sqlite.table.NewsChannelTable;
...
public class NewsChannelDao {
    ...
    public void addInitData() {
        String categoryId[] = VFrame.getResources().getStringArray(R.array.mobile_news_id);
        String categoryName[] = VFrame.getResources().getStringArray(R.array.mobile_news_name);
        for (int i = 0; i < 8; i++) {
            add(categoryId[i], categoryName[i], Constants.NEWS_CHANNEL_ENABLE, i);
        }
        for (int i = 8; i < categoryId.length; i++) {
            add(categoryId[i], categoryName[i], Constants.NEWS_CHANNEL_DISABLE, i);
        }
    }
