...
public class RecyFollowHorizontalAdapter extends BaseQuickAdapter<HomeBean.Issue.Item, BaseViewHolder> {

	@BindView(R.id.iv_cover_feed)
	GlideImageView ivCoverFeed;
	@BindView(R.id.tv_title)
	TextView tvTitle;
	@BindView(R.id.tv_tag)
	TextView tvTag;

	public RecyFollowHorizontalAdapter(@Nullable List<HomeBean.Issue.Item> data) {
		super(R.layout.item_recy_eye_follow_hor, data);
	}
	...
	protected void convert(BaseViewHolder helper, HomeBean.Issue.Item item) {
		...
		if (item != null){
			...
			if (data != null){
				...
				if (cover != null){
					ivCoverFeed.loadImage(cover.getFeed(),R.color.placeholder_color);
				}
			}
		}
	}
