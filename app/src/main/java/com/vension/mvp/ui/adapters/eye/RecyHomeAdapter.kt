...
class RecyHomeAdapter(context: Context, data: ArrayList<HomeBean.Issue.Item>)
    : CommonRecyAdapter<HomeBean.Issue.Item>(context, data, -1) {
                    ...
                    getView<BGABanner>(R.id.banner).run {
                        ...
                        setAdapter(object : BGABanner.Adapter<ImageView, String> {
                            override fun fillBannerItem(bgaBanner: BGABanner?, imageView: ImageView?, feedImageUrl: String?, position: Int) {
                                GlideImageLoader.create(imageView).loadImage(feedImageUrl,R.color.placeholder_color)
                            }
                        })
                    }
                }
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        return when (viewType) {
            ITEM_TYPE_BANNER ->
                ViewHolder(inflaterView(R.layout.item_recy_eye_home_banner, parent))
            ITEM_TYPE_TEXT_HEADER ->
                ViewHolder(inflaterView(R.layout.item_recy_eye_home_header, parent))
            else ->
                ViewHolder(inflaterView(R.layout.item_recy_eye_home_content, parent))
        }
    }
    ...
    private fun setVideoItem(holder: ViewHolder, item: HomeBean.Issue.Item) {
        ...
        // 加载封页图
        GlideImageLoader.create(holder.getView(R.id.iv_cover_feed)).loadImage(cover,R.color.placeholder_color)
        // 如果提供者信息为空，就显示默认
        GlideImageLoader.create(holder.getView(R.id.iv_avatar)).loadImage(avatar,R.color.placeholder_color)
